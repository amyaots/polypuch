...
int main(int argc, char *argv[]) {
	...
	while (go) {
		if (SDL_PollEvent(mainEventPointer)) {
			...
			firstVector.first = SQUARE_MARGIN + rand() % (SCREEN_WIDTH - 2 * SQUARE_MARGIN);
			firstVector.second = SQUARE_MARGIN + rand() % (SCREEN_HEIGHT - 2 * SQUARE_MARGIN);
